...
module.exports = function (content) {
  ...
  if (this.resourceQuery) {
    let queryParams = new URLSearchParams(this.resourceQuery)
  ...
  if (limit <= 0 || content.length < limit) {
    ...
    if (query.encoding === 'base64') {
      data = 'data:image/svg+xml;base64,' + Buffer.from(transformedSvgSource).toString('base64')
    } else {
      data = 'data:image/svg+xml,' + uriEncode(transformedSvgSource)
    }

    if (!query.iesafe || data.length <= 4096 || !REGEX_STYLE.test(originalSvgSource)) {
